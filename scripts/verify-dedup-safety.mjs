...
}).project({ Company: 1, Country: 1, leadId: 1, dedupKeeperLeadId: 1 }).toArray();
...
for (const a of archivedPartners) {
  ...
  console.log(`  ${ok ? '✅' : '⚠️ '} ${a.Company} (${a.Country})`);
...
}).project({ Company: 1, Country: 1, leadId: 1, dedupOriginalStage: 1, dedupKeeperLeadId: 1, emailHistory: 1 }).toArray();
...
const aliveUnique = await Lead.aggregate([
  ...
  { $group: { _id: { c: { $toLower: '$Company' }, co: { $toLower: '$Country' } } } },
  { $count: 'n' },
]).toArray();
