...
const docs = await L.find(LEGACY, {
  projection: { Company: 1, Email: 1, Country: 1, WebsiteContact: 1, createdAt: 1, importBatch: 1 },
...
for (const d of docs) {
  ...
  const key = [company, norm(d.Email), norm(d.Country)].join('||');
...
for (let i = 0; i < ids.length; i += 500) {
  const r = await L.updateMany(
    ...
    { $set: { deleted: true, deletedReason: '기존 데이터 중복 정리 (회사명·이메일·국가 동일)', deletedAt: new Date().toISOString() } },
  );
