 ...
 *  2순위 회사명+국가 — 이름만으로 묶으면 Kosmetrics(브라질·칠레·멕시코),
...
const keepers = await L.find({ stage: 'verified', ...ALIVE })
  .project({ leadId: 1, Company: 1, Country: 1, Email: 1 }).toArray();

const byEmail = new Map();
const byCoCountry = new Map();
for (const k of keepers) {
  const e = norm(k.Email);
  if (e && e.includes('@')) byEmail.set(e, k);
  const c = normCo(k.Company);
  if (c) byCoCountry.set(`${c}::${norm(k.Country)}`, k);
}
console.log(`검증 완료 ${keepers.length}곳 — 이메일 ${byEmail.size} · 회사+국가 ${byCoCountry.size} 기준\n`);
...
}).project({ leadId: 1, Company: 1, Country: 1, Email: 1, stage: 1 }).toArray();
...
for (const l of legacy) {
  ...
  if (e && e.includes('@') && byEmail.has(e)) { match = byEmail.get(e); by = 'email'; }
    ...
    const key = `${c}::${norm(l.Country)}`;
    if (c && byCoCountry.has(key)) { match = byCoCountry.get(key); by = 'company+country'; }
  }
...
console.log(`  이메일 일치 ${byKind.email || 0} · 회사+국가 일치 ${byKind['company+country'] || 0}\n`);

for (const h of hits.slice(0, 25)) {
  console.log(`  [${h.by === 'email' ? '메일' : '이름'}] ${String(h.l.Company).slice(0, 34).padEnd(36)} ${String(h.l.Country || '').slice(0, 14).padEnd(15)} ${h.l.Email || ''}`);
