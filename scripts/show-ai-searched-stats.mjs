...
const byCountry = await Lead.aggregate([
  ...
  { $group: { _id: '$Country', n: { $sum: 1 }, withEmail: { $sum: { $cond: [{ $and: [{ $ne: ['$Email', ''] }, { $ne: ['$Email', null] }] }, 1, 0] } } } },
  { $sort: { n: -1 } },
]).toArray();
console.log('국가별 (총 · 이메일 있음):');
byCountry.forEach(c => console.log(`  ${(c._id || 'none').padEnd(20)} ${String(c.n).padStart(3)} · ${c.withEmail}`));

const seedCheck = await Lead.find({ stage: 'ai-searched', $or: [{ WebsiteContact: /dermaofficial/i }, { WebsiteContact: /venuseurope/i }, { Company: /venus europe/i }, { Company: /derma official/i }] }).project({ Company: 1, Country: 1, WebsiteContact: 1, Email: 1 }).toArray();
console.log('\n=== 시드 URL 확인 ===');
seedCheck.forEach(d => console.log(`  ✅ [${d.Country}] ${d.Company} — ${d.WebsiteContact} — ${d.Email || 'no-email'}`));
