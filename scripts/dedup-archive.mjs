...
// 매칭: Company+Country 소문자 기준
// 보존 우선순위: contacted > partner > emailHistory > verified+beauty-buyer+email > verified+email > verified > verifying+beauty-buyer > verifying > archived > failed
...
function priorityScore(doc) {
  ...
  if (doc.verification?.aiVerdict === 'beauty-buyer') s += 2000;
...
const groups = await Lead.aggregate([
  { $addFields: { _ck: { $toLower: { $trim: { input: { $ifNull: ['$Company', ''] } } } }, _cok: { $toLower: { $trim: { input: { $ifNull: ['$Country', ''] } } } } } },
  { $match: { _ck: { $ne: '' } } },
  { $group: {
    _id: { c: '$_ck', co: '$_cok' },
    n: { $sum: 1 },
