...
function buildPending(q, country) {
  const f = { stage: 'verified', deleted: { $ne: true }, ...REAL_EMAIL };
  if (country && country !== 'All') f.Country = country;
  if (q) {
    const rx = new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
    f.$or = [{ Company: rx }, { Country: rx }, { Email: rx }, { WebsiteContact: rx }];
  }
  return f;
}

const fetchBatch = (pending, skip, limit = 30) =>
  Lead.find(pending).sort({ recoScore: -1, Country: 1, Company: 1 })
    .skip(skip).limit(limit).project({ leadId: 1, Company: 1, Country: 1 }).toArray();
...
const country = byCountry[0]?._id;
const expect = byCountry[0]?.n || 0;

const scoped = buildPending('', country);
