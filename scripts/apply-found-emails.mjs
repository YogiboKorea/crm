...
for (const m of merged) {
  ...
  const lead = await L.findOne({ leadId: m.id }, { projection: { Company: 1, Email: 1, stage: 1, deleted: 1, Country: 1 } });
  ...
  plan.push({ _id: String(lead._id), leadId: m.id, company: lead.Company, country: lead.Country, oldEmail: lead.Email ?? '', newEmail, source: m.confirmedUrl || m.sourceUrl || '', sourceType: m.sourceType, emailKind: m.emailKind });
}
...
console.log('\n넣을 곳 (앞 12곳):');
plan.slice(0, 12).forEach((p) => console.log(`  + [${p.country}] ${p.company} | "${p.oldEmail}" → ${p.newEmail}`));
